package com.gamaset.sonicbot.collector.service.academia.match;
...
import com.gamaset.sonicbot.collector.service.academia.match.scrapper.MatchAcademiaScrapperComponent;
 ...
 */
@Service
public class MatchAcademiaService {
	
	private static final Logger LOG = LogManager.getLogger(MatchAcademiaService.class);

	@Autowired
	private MatchAcademiaScrapperComponent scrapper;
	
	private static final String URL_ACADEMIA = "https://www.academiadasapostas.com/stats/livescores/popup/";

//	@Cacheable(cacheNames = CachingConfig.MATCH_BY_DATE_CHACHE)
	public MatchSeriesDTO listByDate(String date){

		String url = (date == null || date.trim().isEmpty()) ? URL_ACADEMIA : URL_ACADEMIA + DateUtils.convertDateStringToTimestamp(date) + "/";
		
		MatchSeriesDTO matchSeries = scrapper.scrap(getAvailableComps(), url);
		
		LOG.info(String.format("%n===== matches found [%d] =====", matchSeries.getMatches().size()));
	...
	/**
	 * TODO Adicionar Consulta via banco de dados
	 * @return
	 */
